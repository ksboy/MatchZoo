"""RTE(SuperGLUE version) data loader."""
...
_url = "https://dl.fbaipublicfiles.com/glue/superglue/data/v2/RTE.zip"
...
def load_data(
    ...
    :param target_label: If `ranking`, chose one of `entailment`,
        `not_entailment` as the positive label.
        (default: `entailment`)
    ...
    if isinstance(task, matchzoo.tasks.Ranking):
        if target_label not in ['entailment', 'not_entailment']:
            raise ValueError(f"{target_label} is not a valid target label."
                             f"Must be one of `entailment`, `not_entailment`")
...
def _download_data():
    ref_path = keras.utils.data_utils.get_file(
        'RTE_data', _url, extract=True,
        cache_dir=matchzoo.USER_DATA_DIR,
        cache_subdir='RTE'
    )
    return Path(ref_path).parent.joinpath('RTE')
...
if __name__ == "__main__":
    _read_data("/Users/ksboy/Datasets/SuperGLUE/RTE/val.jsonl")
